...
class JLS(object):
    ...
    *keysize* : `int` --
    ...
    def Protect(self, pp, sk_u, tau, x_u_tau, encoded=False):
        """
        ...
        *encoded* : `bool`
            Whether the user's input has been encoded.
...
class TJLS(JLS):
    ...
    *ISS* : `ISSS`
    ...
    def ShareProtect(self, pp, list_sk_v_ushare, tau, z_u_tau=0, user_id=None, online_user_ids=[]):
        """
        Protect a custom value with u's shares of all failed users' secret keys: 
        $$[ y'_{\\tau}]_u \\gets \\textbf{TJL.ShareProtect}(pp,\\{[\\Delta sk_v]_u\\}_{v\\in \\mathcal{U}''},\\tau,\\c_{u, tau})$$

        ### This algorithm protects a custom value with user \\(u\\)s shares of all the secret keys corresponding to the failed users ( \\(v \\in\\mathcal{U}''\\) ) ( \\([\\Delta sk_v]_u\\) is the user \\(u\\) share of the secret key \\(sk_v\\) corresponding to the failed user \\(v\\) ). It basically calls \\(\\textbf{JL.Protect}(pp,\\sum_{v\\in \\mathcal{U}''}[\\Delta sk_v]_u,\\tau, \\c_{u, tau})\\) and outputs \\([y'_{\\tau}]_u = H(\\tau)^{\\sum_{v\\in \\mathcal{U}''}[\\Delta sk_v]_u} \\mod N^2\\). This algorithm is called when there are failed users and hence their input need to be recovered.
        ...
        *z_u_tau*: `int` or `list` --
            The custom input of user u \\(c_{u,\\tau}\\)

        ## **Returns**:
        ----------------
        A share of the protected custom value with all failed users keys of type `IntSecretShare`
        """
        ...
        if z_u_tau == 0:
            if self.VE is not None:
                y_c_ushare_tau =  self.Protect(pp, keyshare, tau, [0] * self.VE.vectorsize)
                r = []
                for yzero_ushare_tau_i in y_c_ushare_tau: 
                    r.append(IShare(idx,yzero_ushare_tau_i))
                return r
            else:
                y_c_ushare_tau = self.Protect(pp, keyshare, tau, 0)
                return IShare(idx, y_c_ushare_tau)
        else:
            assert user_id != None and type(user_id) == int, 'user_id must be an int'
            numerator = 1
            denominator = 1
            for online_user_id in online_user_ids:
                if online_user_id != user_id:
                    numerator *= online_user_id
                    denominator *= (online_user_id - user_id)
            coeff = (self.delta**2) * invert((self.delta * numerator) // denominator, pp.nsquare)
            if isinstance(z_u_tau, list):
                z_u_tau = self.VE.encode(z_u_tau)
                z_u_tau = [coeff * x for x in z_u_tau]
            
            if self.VE is not None:
                assert isinstance(z_u_tau, list) == True, 'z_u_tau must be a vector'
                y_c_ushare_tau =  self.Protect(pp, keyshare, tau, z_u_tau, encoded=True)
                r = []
                for yzero_ushare_tau_i in y_c_ushare_tau: 
                    r.append(IShare(idx,yzero_ushare_tau_i))
                return r
            else:
                y_c_ushare_tau = self.Protect(pp, keyshare, tau, coeff * z_u_tau)
                return IShare(idx, y_c_ushare_tau)
